...
globalThis.Fragment_8vg9x3sq = import_jsx_runtime.Fragment;
globalThis.Fragment = import_jsx_runtime.Fragment;
async function start() {
  const url = new URL(window.location.href);
  const path = url.pathname;
  const Shell = (await import("/" + globalThis.__REACT_SSR_PLUGIN_OPTIONS__.pathToShellFile.replace(".tsx", ".js"))).default;
  const page = await import("/" + join(globalThis.__REACT_SSR_PLUGIN_OPTIONS__.pathToPagesDir, path, "index.js"));
  const layoutRoutes = globalThis.__ROUTES__.filter((path2) => path2.endsWith("layout.js")).map((path2) => "/" + join(globalThis.__REACT_SSR_PLUGIN_OPTIONS__.pathToPagesDir, path2));
  const layouts = await layoutGetter(url.pathname, layoutRoutes, globalThis.__REACT_SSR_PLUGIN_OPTIONS__.pathToPagesDir);
  import_client.hydrateRoot(document.getElementById("root"), /* @__PURE__ */ jsx_dev_runtime.jsxDEV(Shell, {
    request: null,
    children: /* @__PURE__ */ jsx_dev_runtime.jsxDEV(StackLayouts, {
      layouts: layouts.map((_module) => _module.default),
      children: /* @__PURE__ */ jsx_dev_runtime.jsxDEV(page.default, {}, undefined, false, undefined, this)
