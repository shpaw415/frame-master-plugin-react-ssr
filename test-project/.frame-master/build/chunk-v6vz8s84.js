...
function routeGetter(request) {
  return request ? request.getContext().__ROUTES__ : globalThis.__ROUTES__;
}

// src/router/layout.ts
function StackLayouts({ children, layouts }) {
  return layouts.reduceRight((children2, Layout) => {
    return Layout({ children: children2 });
  }, children);
}
async function importLayouts(layoutsPaths) {
  if (globalThis.process.env.NODE_ENV === "development") {
    const searchParams = new URLSearchParams;
    searchParams.set("t", new Date().getTime().toString());
    layoutsPaths = layoutsPaths.map((layoutPath) => layoutPath + "?" + searchParams.toString());
  }
  return await Promise.all(layoutsPaths.map((layoutPath) => import("/" + join(globalThis.__REACT_SSR_PLUGIN_OPTIONS__.pathToPagesDir, layoutPath))));
}
function getRelatedLayoutPaths(path, layouts, basePath) {
  ...
  const rootLayout = normalizedLayouts.find((layout) => rootLayoutRegex.test(layout));
  if (rootLayout) {
  ...
  for (const segment of segments) {
    ...
    const matchedLayout = normalizedLayouts.find((layout) => layoutRegex.test(layout));
    if (matchedLayout) {
...
function layoutGetter(currentPath, layoutsList, basePath) {
  const related = getRelatedLayoutPaths(currentPath, layoutsList, basePath);
  return importLayouts(related);
}

export { require_react, join, routeGetter, require_jsx_dev_runtime, StackLayouts, layoutGetter };
